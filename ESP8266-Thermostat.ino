...
String generateTemplateKeyValuePairs (const String& key) {
    switch(key) {
        case "CURRENT_TEMP": return String(temp_f, DEC),
        case "TARGET_TEMP": return String(targetTemperature, DEC),
        case "POLL_INTERVALMS": return String(pollInterval, DEC),
        case "POLL_INTERVAL": return String(pollInterval / 1000, DEC),
        case "HEAT_STATUS": return String(heatOn ? "ON" : "OFF"),
        case "HEAT_RUN": return heatOn ? String("Furnace has been running for " + String(heatRunningFor(), DEC) + "seconds") : String("Furnace last ran for " + String(heatLastRanFor, DEC) + "seconds"),
        default: return ""
    }
}
...
void handleRoot() {
    if (ESPTemplateProcessor(server).send(String("/template_root.htmlt"), generateTemplateKeyValuePairs)) {
        ...
        server.send(500, "text/plain", "<h1>Internal Server Error</h1><p>The author of this application is an idiot, and " + 
            "now you are paying the price.  In fairness so is the President of the United States so... your fault too?</p>");
    }
}
...
void handleUpdate() {
    ...
    if (response == "") {
        ...
        if (tempTgt > 0 && tempTgt != temperatureTarget) {
            temperatureTarget = tempTgt;
...
void considerFurnaceStateChange() {
    if ((temp_f <= temperatureTarget) && !heatOn) {
        turnHeatOn();
        Serial.println("[info] below temperatureTarget; turning ON furnace..");
    } else if (heatOn && (temp_f >= (temperatureTarget + ttSkew))) {
        turnHeatOff();
        Serial.println("[info] temperatureTarget met; turning OFF furnace..");
    } else {
...
String contentType(String filename){
  ...
  else if(filename.endsWith(".htmlt")) return "text/html";
